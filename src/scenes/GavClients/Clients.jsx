...
const Clients = () => {
  ...
  const fetchClientData = async () => {
    ...
    try {
      ...
      if (usertype === "CREDIX_ADMIN") {
        ...
        // };
        payload = {
          serviceReference: "GET_CLIENT_ACCOUNTS_BY_ID",
          requestBody: spaceId,
          spaceId: spaceId,
        };
      }
  ...
  const handleConfirmActivate = async () => {
    ...
    try {
      ...
      if (response && response.body.meta.statusCode === 200) {
        ...
        // setErrorMessage(response.body.errors || 'Unauthorized to perform action');
        showSnackbar(
          response.body.errors || "Unauthorized to perform action",
          "error"
        );
      } else {
        showSnackbar(
          response.body.errors || "Error Activating client",
          "error"
        );
      }
    } catch (error) {
        ...
        return (
          <>
            ...
            <Menu
              ...
              anchorEl={anchorEl}
              open={
                Boolean(anchorEl) && currentRow?.msisdn === params.row.msisdn
              }
              onClose={handleClose}
              PaperProps={{
                style: {
  ...
  return (
    <Box m="20px">
      ...
      <Dialog
        open={showActivateClientModal}
        onClose={() => handleToggleActivateClientModal(false)}
        maxWidth="sm"
        fullWidth
      >
        ...
        <DialogActions>
          <Button
            onClick={() => handleToggleActivateClientModal(false)}
            color="primary"
            variant="contained"
          >
            Cancel
