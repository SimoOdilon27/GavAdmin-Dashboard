...
const ChargesForm = () => {
    ...
    const handleFormSubmit = async (values) => {
        ...
        try {
            ...
            if (response && response.body.meta.statusCode === 200) {
                showSnackbar(id ? 'Charge Updated Successfully.' : 'Charge Created Successfully.', 'success');
                setTimeout(() => {
                    navigate('/charges');
                }, 2000);
            } else {
                showSnackbar(response.body.errors || (id ? 'Error Updating Charge' : 'Error Creating Charge'), 'error');
            }
        } catch (error) {
            console.error("Error:", error);
    ...
    return (
        <Box m="20px">
            <Header
                ...
                subtitle={id ? "Edit the charge configuration" : "Add a new charge configuration"}
            />
                            ...
                            >
                                ...
                                {usertype === ("CORPORATION_ADMIN") &&
                                    <FormControl fullWidth variant="filled"
                                        ...
                                        >
                                            <MenuItem value="" selected disabled>Select CBS Branch ID</MenuItem>
                                            {Array.isArray(bankIDbyCorp) && bankIDbyCorp.length > 0 ? (
                                ...
                                <TextField
                                    ...
                                    error={!!touched.operationConfigId && !!errors.operationConfigId}
                                    helperText={touched.operationConfigId && errors.operationConfigId}
                                    sx={FormFieldStyles("span 2")}
                                />
                                <TextField
                                    fullWidth
                                    ...
                                    <FormControlLabel
                                        control={
                                            <Checkbox
                                                ...
                                                onChange={(e) => setFieldValue('isChargePercentage', e.target.checked)}
