...
const OtherBankInvestments = () => {
  ...
  const [awaitingInvestmentApprovalData, setAwaitingInvestmentApprovalData] =
    useState([]);
  ...
  const handleApproveInvestment = async (id) => {
    try {
      ...
      if (response && response.body.meta.statusCode === 200) {
        ...
        showSnackbar(
          response.body.errors || "Approval Investment Failed.",
          "error"
        );
      }
    } catch (error) {
      ...
      setLoadingRows((prevLoadingRows) =>
        prevLoadingRows.filter((rowId) => rowId !== id)
      );
    }
  };
    ...
    <Box m="20px">
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            onClick={handleConfirmApproveInvestment}
            color="secondary"
            variant="contained"
          >
            Confirm
