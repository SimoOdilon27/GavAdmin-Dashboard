...
const BankAccount = () => {
  ...
  const handleActivateDeactivate = (accountId, action) => {
    setSelectedAccount(
      bankAccountData.find((account) => account.accountId === accountId) || null
    );
    setAction(action);
  ...
  const confirmActivateDeactivate = async () => {
    try {
      ...
      if (response && response.body.meta.statusCode === 200) {
        ...
        showSnackbar(
          response.body.errors || `Error ${action}ing account`,
          "error"
        );
      }
    } catch (error) {
  ...
  const handleViewAccount = (row) => {
    ...
    navigate(`/bankaccount/view/${row.accountId}`, {
  ...
  const columns = [
    ...
    {
      field: "totalCapitalInvested",
      headerName: "Total Amount Invested",
      flex: 1,
      headerAlign: "center",
      align: "center",
      valueGetter: (params) => formatValue(params.value),
        ...
        return (
          <>
            ...
            <Menu
              ...
              open={
                Boolean(anchorEl) &&
                currentRow?.accountId === params.row?.accountId
              }
              onClose={handleClose}
              PaperProps={{
                style: {
    ...
    <Box m="20px">
      ...
      >
        <DataGrid
          rows={
            selectedTab === 0
              ? filterDataByType("OPERATION")
              : selectedTab === 1
              ? filterDataByType("COMPENSATION")
              : selectedTab === 2
              ? filterDataByType("COMMISSION")
              : selectedTab === 3
              ? filterDataByType("GIMAC")
              : selectedTab === 4
              ? filterDataByType("TAX")
      ...
      >
        <DialogTitle>
          {action === "activate" ? "Activate Bank" : "Deactivate Bank"}
        </DialogTitle>
        <DialogContent>
